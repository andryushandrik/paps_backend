...
export class TokenService {
    ...
    private generateAccessToken(user: Omit<User, 'password'>): Token {
        const payload = {
            id: user.id,
            email: user.email,
            name: user.name,
    ...
    private generateRefreshToken(user: Omit<User, 'password'>): Token {
        const payload = {
            id: user.id,
            email: user.email,
            type: 'refresh_token',
    ...
    public async createTokensAndSession(user: Omit<User, 'password'>, browserDataDto: BrowserDataDto): Promise<Tokens> {
